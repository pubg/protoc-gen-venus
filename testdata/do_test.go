	...
	for _, dir := range dirs {
		...
		t.Run(dir.Name(), func(t *testing.T) {
			...
			for _, artifact := range artifacts {
				...
				actual, err := convertToComparableComponent([]byte(fArtifact.Contents))
				if err != nil {
...
func newModule(tc *Testcase, dirPath string) (*modules.VenusModule, pgs.MockDebugger) {
	debugger := pgs.InitMockDebugger()
	module := modules.NewVenusModule()
...
func loadExpectedResult(dirPath string, files []string) (*ExpectedResult, error) {
	...
	for _, file := range files {
		...
		components, err := convertToComparableComponent(buf)
		if err != nil {
			return nil, err
		}
		result.results[file] = components
	}
	return result, nil
}
